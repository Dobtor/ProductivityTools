...
class SurveyHistory(models.Model):
    _name = 'dobtor.todo.list.user_input.ref'

    @api.depends('todo_list_ids')
    def _compute_response_id(self):
        for recode in self:
            recode.reject = recode.todo_list_ids.response_id

    todo_list_ids = fields.Many2one(
...
class SurveyUserInput(models.Model):
    ...
    todo_list_user_input_ids = fields.One2many(
        string='user_input',
        comodel_name='dobtor.todo.list.user_input.ref',
