...
(function () {
    ...
    class MindMapView {
        ...
        _buildDOM() {
            ...
            // Pan/zoom events — 保留 bound 參照供 destroy() 移除，避免 document 上的
            // mousemove/mouseup 監聽在反覆 re-render 時累積洩漏。
            this._boundPanStart = (e) => this._onPanStart(e);
            this._boundPanMove = (e) => this._onPanMove(e);
            this._boundPanEnd = () => this._onPanEnd();
            this._boundWheel = (e) => this._onWheel(e);
            this.container.addEventListener('mousedown', this._boundPanStart);
        ...
        /** 移除掛在 document / container 上的事件監聽並釋放參照。 */
        destroy() {
            if (this._boundPanMove) {
                document.removeEventListener('mousemove', this._boundPanMove);
            }
            if (this._boundPanEnd) {
                document.removeEventListener('mouseup', this._boundPanEnd);
            }
            if (this.container) {
                if (this._boundPanStart) {
                    this.container.removeEventListener('mousedown', this._boundPanStart);
                }
                if (this._boundWheel) {
        ...
        /**
         * 自動縮放/平移，使目前所有「可見」節點恰好填滿容器（留 pad 邊距）。
         * 座標：節點以 world 座標 (_x,_y) 中心 + (_w,_h) 尺寸；world 經
         * transform `translate(panX+cx, panY+cy) scale(zoom)` 投影，world 原點
         * 對齊容器中心，故 pan = -zoom * 內容中心 即置中。
         */
        fitToContainer(pad) {
            ...
            const walk = (n) => {
                ...
                if (n._el && n._el.style.display !== 'none' &&
                    typeof n._x === 'number' && typeof n._w === 'number') {
            ...
            let z = Math.min(cw / contentW, ch / contentH);
        ...
        begin_edit(node) {
            ...
            // 唯讀圖（如 mail.activity 關聯圖 editable:false）不允許雙擊改節點名
    ...
    class OdooMindMap {
        ...
        /** 卸載：移除視圖層掛在 document 上的監聽並釋放參照（防洩漏）。 */
        destroy() {
        ...
        // === Zoom / Fit passthrough（供外部工具列按鈕使用）===
        getZoom() { return this.view ? this.view.getZoom() : 1; }
        setZoom(z) { if (this.view) this.view.setZoom(z); }
        zoomIn(step) { if (this.view) this.view.setZoom(this.view.getZoom() * (step || 1.2)); }
        zoomOut(step) { if (this.view) this.view.setZoom(this.view.getZoom() / (step || 1.2)); }
        resetView() { if (this.view) this.view.resetView(); }
        fitToContainer(pad) { if (this.view) this.view.fitToContainer(pad); }
