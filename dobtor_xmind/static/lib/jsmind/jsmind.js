...
(function (factory) {
    ...
    class LayoutProvider {
        ...
        _layout_node(node, direction) {
            // Get existing layout_data or create new one
            ...
            // Only set offset_x if not already set by _layout_children_nodes
            if (direction === 2) {
                if (!layout_data.offset_x) layout_data.offset_x = 0;
                if (!layout_data.offset_y) layout_data.offset_y = this.jm.view.layout.vspace;
            } else {
                if (!layout_data.offset_x) layout_data.offset_x = this.jm.view.layout.hspace * direction;
            }
        ...
        _layout_node_vertical(node) {
            // Preserve existing layout_data to avoid overwriting offset_x set by parent
            const layout_data = node._data.layout || {};
            ...
            // Only set offset_y if not already set
            if (!layout_data.offset_y) {
        ...
        _layout_offset() {
            ...
            // Set root node absolute position
            layout_data.abs_x = 0;
            layout_data.abs_y = 0;
