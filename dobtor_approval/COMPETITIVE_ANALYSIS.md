...
> 文件版本：v1.6 ｜ 撰寫日：2026-06-06（v1.5）｜ 更新：2026-06-08（v1.6 DMN 子系統完整落地）
> 分析基準：`DESIGN.md`（M1–M7）+ `DESIGN_SELF_SERVICE_DESIGNER.md`（自助設計器）+ `DESIGN_PROGRESSIVE_TIERS.md`（能力分級 L1–L4＋專家包）+ `DESIGN_DMN.md`（DMN 決策子系統）+ `DESIGN_MODULE_SPLIT.md`（設計/執行分層）全數完工的
...
> **v1.5 變更**：收斂為 **2 模組**——原子擴充 `dobtor_approval_activity`（代簽核/加簽）、`dobtor_approval_dmn`（DMN 求值）**併入 `dobtor_approval`**，改以模組內能力開關（預設關閉）控制，不再實體拆分子模組。
> **v1.6 變更（2026-06-08，重大）**：DMN 從「Roadmap/部分」升為**完整實作**，多項對標評分上修：
> ① **DMN 決策子系統完整落地**：自寫 **FEEL 求值引擎**（純 Python，48 單元測試）＋決策表＋**DRD 多決策依賴編排**＋BKM＋hit policy（unique/collect/priority…）＋覆蓋分析＋試算。
> ② **核決權限表（Authority Matrix）**：金額/類別/部門 → 簽核人鏈，依序逐關核決——**正面對標 FlowMaster/華苓「核決權限表」強項，且可一鍵轉 DMN**。
> ③ **DMN↔BPMN 三整合出口**：簽核節點以 DMN 決策解析簽核人鏈、**商業規則任務（businessRuleTask）求值寫回**、**互斥/包容閘道以 DMN 決策路由（注入 dmn_result）**。
> ④ **bpmn-js / dmn-js 直接打包進模組（vendored）**：不再依賴 CDN，**完全離線可用**、無 404 噪音。
> ⑤ **閘道引擎強化**：互斥（條件＋AND/OR 群組＋預設線）、**包容/並行閘道 join 同步（可達性分析，消 over-sync、無死結）**。
> ⑥ **攔截閘門方法「用選的」**：自助 widget 讀目標模型 form 按鈕 → 下拉中文按鈕名，免查 code 手打方法名。
> ⑦ **自動化測試**：新增 5 支 Odoo 測試（DMN 解析/DRD 求值/hit policy/閘道 join/簽核節點/規則任務/路由）。
> ⑧ 能力分級由 T0–T6 工程視角重構為 **L1–L4 客群視角＋專家包＋SI 實作者群組**（見 `DESIGN_PROGRESSIVE_TIERS.md` v2.0）。
> ⚠ 誠實註記：以上 v1.6 功能皆已**程式完成＋靜態/單元驗證**，惟尚待在實機 Odoo 實例端到端驗證（並發 join 行為、前端互動、權限）。
...
|---|---|---|---|---|---|---|---|---|
| BPMN 2.0 可執行引擎 | ✅ 子集+businessRuleTask+閘道join(可達性) | ✅ | 🟡 | 🟡 自有引擎(非BPMN) | ✅✅ 標竿 | ✅ | ✅ | ⚠ 非BPMN |
| DMN 決策引擎 | 🔵✅✅ 自寫FEEL引擎+決策表+DRD+BKM(dmn-js vendored) | 🟡 | ⚠ | 🟡 條件判斷(AutoScript) | ✅✅ | ✅✅ | ✅ | 🟡 |
| **核決權限表(金額/類別/部門→簽核鏈)** | 🔵✅✅ Authority Matrix+依序逐關+可轉DMN | ✅ | 🟡 | ✅✅ 核決權限表 | 🟡 需建模 | ✅ | ✅ | ⚠ |
| 流程模擬/挖掘 | 🟡 沙箱dry-run+DMN試算(無挖掘) | ⚠ | ⚠ | ✅ Simulator+Monitor分析 | 🟡 | ✅ | ✅✅ | 🟡 |
| **上手簡單度/漸進式複雜度** | 🔵✅✅ 由簡入深(L1開箱·開關啟用) | 🟡 | 🟡 | 🟡 完整平台 | ⚠ 陡 | ⚠ 陡 | ✅ | ✅ |
...
| 視覺流程編輯器 | ✅✅ bpmn-js+dmn-js**內建打包(離線)**+面板+lint+模擬+內嵌試算 | ✅ 自有 | ✅ | ✅ Designer 自有 | ✅✅ bpmn-js原廠 | ✅ | ✅ | ✅ |
...
- 導入 **L1–L4 客群分級 + 專家包 + 進階功能預設關閉的開關架構**（沿用 Dobtor-HR「開關 + Security Group」策略；見 `DESIGN_PROGRESSIVE_TIERS.md` v2.0）：
  - **開箱 = L1**：精簡精靈 + 「開始→簽核→結束」+ 六種簽核人 + 任一核准，5 分鐘建第一條請假流程，不被複雜度淹沒。
  - **按需長出**：要條件分歧/會簽開 L2、要綁 ERP 動作開 L3、要視覺自助/治理開 L4、要 **DMN/核決權限表/商業規則任務**開**專家包** —— 每一步一個開關，SI 實作者可橫向加開。
...
### 🔵 8.（v1.6 新增·已落地）核決權限表 × DMN 決策 —— 從「Roadmap」變「實裝」
- **核決權限表（Authority Matrix）**：台灣企業最高頻的「金額/類別/部門 → 該找誰簽、簽到哪一層」需求，做成**可編輯、可版本、可稽核的結構化決策表**，命中各級**依序逐關核決**（直屬→部門經理→上層）。這正是華苓/**新人類 FlowMaster 的招牌「核決權限表」**——dobtor_approval 現在**有對等能力，且能一鍵轉成通用 DMN 決策**。
- **完整 DMN 決策子系統**：自寫 **FEEL 求值引擎**（純 Python、48 單元測試、無 `eval`）＋決策表＋**DRD 多決策依賴編排**（風險評等→核決層級鏈→共用 BKM）＋hit policy＋覆蓋/缺口分析＋試算。**不只「條件判斷」，而是可組合的決策層。**
- **DMN 與 BPMN 三個整合出口**：① 簽核節點以 DMN 決策動態解析簽核人鏈 ② **商業規則任務**求值後寫回單據/流程變數 ③ **互斥/包容閘道以 DMN 決策路由**（`dmn_result` 注入條件）。
- 與國際引擎差異：Camunda/Pega 的 DMN 強，但**不懂台灣核決權限表語意、不長在 Odoo 體內**；FlowMaster 懂核決權限表，但**是自有引擎、非 DMN 開放標準、對 Odoo 仍是 connector**。**「核決權限表語意 × DMN 開放標準 × Odoo 零距離」三者交集，仍只有 dobtor_approval。**
- ⚠ 誠實：DMN 子系統程式與單元測試完成，**待實機端到端驗證**。
...
6. **營運級流程監控**：Monitor（逾期分析、關卡效率、簽核時間統計）；dobtor 僅沙箱 dry-run + DMN 試算、無營運分析。
...
### v1.6 補強：核決權限表落差已縮小
- FlowMaster 的招牌之一是**核決權限表**（金額分層核決）。v1.6 前 dobtor_approval 靠 HR resolver 近似；**v1.6 已內建 Authority Matrix + DMN 決策子系統**，提供對等的「金額/類別/部門→依序逐關核決」，並多出 **DMN 開放標準 + 一鍵轉換 + 與 Odoo 單據零距離求值** 三項 FlowMaster 沒有的優勢。
- 結論不變：表單設計器/SAP整合/行動App/AI 成熟度仍是 FlowMaster 主場；但**「核決權限表 + ERP 內簽核」這塊，dobtor_approval 已從追趕轉為對等且更開放**。
...
### 🟡 2.（持續補齊中）BPMN 覆蓋度 — 核心子集 + 進階可選擴充
- **v1.0 原弱項**：只做 UserTask / ServiceTask / 三種 Gateway / Start-End。
- **已實裝**：UserTask（含會簽/依序/加簽/代理）、ServiceTask、**BusinessRuleTask（DMN 求值寫回）**、互斥閘道（條件＋AND/OR 群組＋預設線）、**包容/並行閘道含 join 同步（可達性分析，消 over-sync、無死結）**、**DMN 決策表/DRD**。
- **能力開關可選**：Timer/Boundary（SLA 逾時）、Error/Incident（失敗重試）、真 Multi-instance、Call Activity 等仍為骨架/Roadmap（依分級開關）。
- **仍誠實不做**：Compensation（補償交易自動回滾）、Message Correlation/Choreography（事件驅動跨流程編排）、執行中實例 live migration —— 超出「企業內部簽核」定位，與 Camunda/Pega 仍有本質差距。
- → 結論：核心執行 + DMN 決策 + 閘道 join 已落地；**仍非完整 conformance-class 引擎**，但對「企業內部簽核 + 核決決策」已足。

### ⚠ 3. 流程挖掘 / AI 決策 仍缺（DMN 規則決策已有）
- **已有**：DMN **規則式決策**（決策表/DRD/FEEL）—— 可表達「依金額/類別/部門/風險判斷」的結構化商業規則。
- **仍缺**：Celonis/Appian 的 **Process Mining**（流程挖掘）、Pega 的 **AI Decisioning**（機器學習/預測式決策）。
- → 能「執行流程 + 規則決策」，但無法「**從歷史資料挖掘瓶頸、預測、最佳化**」。這是高階 BPM 的智慧化價值區，短期補不上（可後接 LLM 做設定助手/簽核摘要作差異化）。
...
| **已用 Odoo，要金額/類別/部門分層核決（核決權限表）** | 🏆 **dobtor_approval** | v1.6 內建 Authority Matrix + DMN，依序逐關核決，與單據零距離 |
...
| 台灣簽核文化貼合 | **5** 🔵 | 5 | **5** | 2 | 3 |
| 引擎規模/擴展 | 2 ⚠ | 3 | 3 | **5** | 4 |
| BPMN 標準完整度 | **4** *(子集+DMN+businessRuleTask+閘道join)* | 4 | 3 *(自有引擎)* | **5** | 4 |
| DMN/規則決策完整度 | **4** 🔵 *(FEEL引擎+DRD+核決權限表)* | 2 | 1 | **5** | **5** |
| 智慧化(挖掘/AI) | 2 *(規則決策有·挖掘/AI缺)* | 2 | 3 *(智慧精靈+Monitor)* | 3 | **5** |
...
> **v1.6 上修**：新增「DMN/規則決策完整度」列（dobtor 4 🔵，僅次 Camunda/Pega）；「智慧化」由 1→2（規則決策已有，挖掘/AI 仍缺）；BPMN 標準完整度註記補 DMN/businessRuleTask/閘道 join。**核決權限表落差由「追趕」轉為「對等且更開放」。**
> 但 dobtor_approval 仍在 **「ERP整合(5) / 在地簽核(5) / 低成本(5) / 公民開發(4) / 漸進式上手(5) / DMN決策(4🔵)」六軸領先或並列前段**；落後集中在「引擎規模 / 挖掘AI / 生態 / 成熟度 / 表單行動」。
